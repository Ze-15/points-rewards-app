...
| # | 场景 | 关键断言 |
|---|---|---|
| 1 | 注册 / 登录 / 登出 / 重复邮箱 / 错误密码 | 注册后余额为 100；重复邮箱 409；错误密码 400 |
| 2 | 领取任务奖励 | 余额 +10，产生 1 条流水，快照正确 |
| 3 | 重复领取（每日 + 一次性） | 返回 `already_claimed`，余额与流水不变 |
| 4 | **10 并发领取同一任务** | **恰好 1 次成功，9 次已领取，只加一次分** |
| 5 | 余额充足时兑换 | 返回 `delivered`，扣 50，产生 1 条支出流水 |
| 6 | **同一幂等键 10 并发提交** | **只 1 条兑换记录、只扣 1 次分、只 1 条流水** |
| 7 | 余额不足兑换 | 400 + `INSUFFICIENT_POINTS`，**无扣分、无流水、无记录** |
| 8 | **并发兑换导致余额不足** | **余额永不为负，成功次数 = 余额可承受次数** |
| 9 | 商品发放失败 | 全额退款，1 条支出 + 1 条退款流水，重试不重复退 |
| 10 | 收支明细逐笔核对 | 流水累计 = 当前余额；每笔快照可逐笔核对 |
| 11 | 未登录访问受保护接口 | 全部返回 401 |
| 12 | **【安全】幂等键按用户隔离** | **B 用与 A 相同的键，拿到的是自己的凭据，读不到 A 的记录** |
